...
void setup() {
  ...
  Serial1.begin(9600);// wind sensor
  //Serial1.flush();  
  ...
  Serial3.begin(38400); //Pins 15(RX), 14(TX), communication with motor arduino
  // setGPS();
  setRadio();
}
...
int boatDir(){
  ...
  if(s_cDataUpdate)
  {
    ...
    {
      //double heading = atan2(sReg[HY], sReg[HX]) * 180 / PI;
      s_cDataUpdate &= ~MAG_UPDATE;
      return atan2(sReg[HY], sReg[HX]) * 180 / PI;
    }
    s_cDataUpdate = 0;
  }
}
...
void loop() {
  ...
  if (radio.available()) {
    ...
    // if (instr.rtk){
    //   //Serial2.print(instr.rtcm);
    //   GPS.pushRawData((uint8_t *)instr.rtcm, 10);
    //   Serial.println(instr.rtcm);
    // }
